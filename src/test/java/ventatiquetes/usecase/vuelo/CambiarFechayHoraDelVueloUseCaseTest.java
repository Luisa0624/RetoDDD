...
import ventatiquetes.domain.vuelo.event.FechayHoraCambiada;
...
class CambiarFechayHoraDelVueloUseCaseTest {

    private CambiarFechayHoraDelVueloUseCase cambiarFechayHoraDelVueloUseCase;
    @Mock
    private DomainEventRepository repository;

    @BeforeEach
    public void setup(){
        cambiarFechayHoraDelVueloUseCase= new CambiarFechayHoraDelVueloUseCase();
        repository=mock(DomainEventRepository.class);
        cambiarFechayHoraDelVueloUseCase.addRepository(repository);
    }

    @Test
    void cambiarFechaYHorahappyPath(){
        //arrage
        var command = new CambiarFechayHora(
                VueloId.of("xxx-xxx"),
                new FechaHora("21/06/2021 15:00")
        );
        ...
        var response= UseCaseHandler.getInstance().
                ...
                cambiarFechayHoraDelVueloUseCase, new RequestCommand<>(command)
        ).orElseThrow();

        var event=(FechayHoraCambiada)response.getDomainEvents().get(0);

        //Asserts:
        Assertions.assertEquals("21/06/2021 15:00", event.getFechahora().value());
