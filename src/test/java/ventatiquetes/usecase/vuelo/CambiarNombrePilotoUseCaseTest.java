...
import ventatiquetes.domain.vuelo.event.NombrePilotoCambiado;
...
class CambiarNombrePilotoUseCaseTest {

    private CambiarNombrePilotoUseCase cambiarNombrePilotoUseCase;
    @Mock
    private DomainEventRepository repository;

    @BeforeEach
    public void setup(){
        cambiarNombrePilotoUseCase= new CambiarNombrePilotoUseCase();
        repository=mock(DomainEventRepository.class);
        cambiarNombrePilotoUseCase.addRepository(repository);
    }

    @Test
    void cambiarNombrePilotoHappyPath(){
        //arrage
        var command = new CambiarNombrePiloto(
                new NombrePiloto("Mario Casas"),
                VueloId.of("xxx-xxx")
                );
        ...
        var response= UseCaseHandler.getInstance().
                ...
                cambiarNombrePilotoUseCase, new RequestCommand<>(command)
        ).orElseThrow();

        var event=(NombrePilotoCambiado)response.getDomainEvents().get(0);

        //Asserts:
        Assertions.assertEquals("Mario Casas", event.getPiloto().value());
